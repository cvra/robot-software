...
We wanted to develop a new system based on \gls{uwb} transceiver technology that would be able to operate inside.
...
* Extraction to UWB
* Kalman filter and variations

%jWhen looking at the litterature we observed that the problem of localization using inertial and position data was quite common.
%jTypical approaches to this problem uses a Kalman filter \cite{titterton2004strapdown}.
%jThis estimator uses the inertial data as the control input and the GPS data as the measurement.
%jA good introduction to the Kalman filter and its non-linear variation, the \gls{ekf} can be found in \cite{thrun2005probabilistic}.
