...
{
    mock("spi").actualCall("send").withPointerParameter("drv", p)
               .withMemoryBufferParameter("buf", (const unsigned char *)buf, n);
}

void spiReceive(SPIDriver *p, size_t n, void *buf)
{
    mock("spi").actualCall("receive").withPointerParameter("drv", p)
               .withParameter("n", n)
               .withOutputParameter("buf", (unsigned char *)buf);
}
