...
{
    (void) readlength;
    mock("spi").actualCall("read")
               .withMemoryBufferParameter("header", (const unsigned char *)headerBuffer, headerLength)
               .withOutputParameter("buf", (unsigned char *)readBuffer);
...
TEST(Decawave, ReadId)
{
    ...
    mock("spi").expectOneCall("read")
                .withMemoryBufferParameter("header", header, sizeof(header))
                .withOutputParameterReturning("buf", reply, sizeof(reply));
