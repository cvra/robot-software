...
void beacon_cartesian_convert(struct robot_position* robot_pos,
                              ...
                              float* y)
{
    *x = distance * cosf(position_get_a_rad_float(robot_pos) + angle) + position_get_x_float(
        robot_pos);
    *y = distance * sinf(position_get_a_rad_float(robot_pos) + angle) + position_get_y_float(
        robot_pos);
}
