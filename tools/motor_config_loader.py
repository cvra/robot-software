...
class ConfigUploader:
    ...
    def _node_status_callback(self, event):
        ...
        if board not in self.boards_id_by_name.values():
            self.node.request(
                uavcan.protocol.GetNodeInfo.Request(), board, self._board_info_callback
            )
    ...
    def send_parameter(self, name, value, board_name):
        with self.node_lock:
            ...
            self.node.request(
                request,
                self.boards_id_by_name[board_name],
    ...
    def _save_parameters_callback(self, event):
        ...
        if not event.response.ok:
            raise RuntimeError(
                "Could not save config on board {}".format(
    ...
    def upload_config(self, configs):
        ...
        for board, config in configs.items():
            for param, value in parameters_from_config(config):
                logging.debug("Sending {} = {} to {}".format(param, value, board))
...
def parse_args():
    ...
    parser.add_argument(
        "-b", "--board", action="append", help="Boards to consider, defaults to all"
    )
