...
#define _EXCVT                                                                                  \
    {                                                                                           \
        0x80, 0x81, 0x45, 0x41, 0x84, 0x41, 0x86, 0x43, 0x45, 0x45, 0x45, 0x49, 0x49, 0x8D,     \
...
#define _EXCVT                                                                                  \
    {                                                                                           \
        0x80, 0x9A, 0x90, 0xB6, 0x8E, 0xB7, 0x8F, 0x80, 0xD2, 0xD3, 0xD4, 0xD8, 0xD7, 0xDE,     \
...
#define _EXCVT                                                                                  \
    {                                                                                           \
        0x80, 0x9A, 0x90, 0xB6, 0x8E, 0xB7, 0x8F, 0x80, 0xD2, 0xD3, 0xD4, 0xD8, 0xD7, 0x98,     \
...
#define _EXCVT                                                                                  \
    {                                                                                           \
        0x80, 0x81, 0x82, 0x82, 0x84, 0x85, 0x86, 0x87, 0x88, 0x89, 0x8A, 0x8B, 0x8C, 0x8D,     \
        ...
        if (c == DDE || ((a & AM_VOL) && a != AM_LFN)) { /* An entry without valid data */
            ...
            if (a == AM_LFN) { /* An LFN entry is found */
                if (dp->lfn) {
                    ...
                    ord =
                        (c == ord && sum == dir[LDIR_Chksum] && cmp_lfn(dp->lfn, dir)) ? ord - 1 : 0xFF;
                }
    ...
    while (dp->sect) {
        ...
        if (c == DDE || (!_FS_RPATH && c == '.') || (int)(a == AM_VOL) != vol) { /* An entry without valid data */
            ...
            if (a == AM_LFN) { /* An LFN entry is found */
                ...
                ord =
                    (c == ord && sum == dir[LDIR_Chksum] && pick_lfn(dp->lfn, dir)) ? ord - 1 : 0xFF;
            } else { /* An SFN entry is found */
                    ...
                    if (res == FR_OK) {
                        ...
                        if (djo.sclust != djn.sclust && (dir[DIR_Attr] & AM_DIR)) { /* Update .. entry in the directory if needed */
                            ...
                            if (!dw) {
                                ...
                                if (res == FR_OK && dir[1] == '.') {
                                    dw =
                                        (djo.fs->fs_type == FS_FAT32 && djn.sclust == djo.fs->dirbase) ? 0 : djn.sclust;
                                    st_clust(dir, dw);
                                    djo.fs->wflag = 1;
                                }
                            }
