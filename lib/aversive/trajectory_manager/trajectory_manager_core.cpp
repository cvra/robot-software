...
void __trajectory_goto_d_a_rel(struct trajectory* traj, double d_mm, double a_rad, enum trajectory_state state, uint8_t flags) EXCLUSIVE_LOCKS_REQUIRED(traj->lock_)
{
    ...
    DEBUG("Goto DA/RS rel to d=%f a_rad=%f", d_mm, a_rad);
    delete_event(traj);
    traj->state = state;
    if (flags & UPDATE_A) {
        if (flags & RESET_A) {
...
void trajectory_manager_xy_event(struct trajectory* traj) EXCLUSIVE_LOCKS_REQUIRED(traj->lock_)
{
    ...
    switch (traj->state) {
        ...
        case RUNNING_XY_B_START:
            /* START -> ANGLE */
            DEBUG("-> ANGLE");
            traj->state = static_cast<enum trajectory_state>(static_cast<int>(traj->state) + 1);
            break;
    ...
    EVT_DEBUG("EVENT XY d_cur=%d, d_consign=%d, d_speed=%d, a_cur=%d, a_consign=%d, a_speed=%d",
...
static void trajectory_manager_line_event(struct trajectory* traj) EXCLUSIVE_LOCKS_REQUIRED(traj->lock_)
{
    ...
    EVT_DEBUG("target.x=%2.2f target.y=%2.2f, a_consign=%d, d_consign=%d",
                           ...
                           uint8_t flags)
{
    ...
    DEBUG("Circle rel (x,y)=%2.2f,%2.2f r=%2.2f flags=%x dst_angle=%d",
...
static void __trajectory_line_abs(struct trajectory* traj,
                                  ...
                                  double x2,
                                  double y2,
                                  double advance) EXCLUSIVE_LOCKS_REQUIRED(traj->lock_)
{
    point_t p1, p2;
    ...
    DEBUG("Line rel (a,b,c)=%2.2f,%2.2f,%2.2f",
                           ...
                           double* remain_d_mm_out)
{
    ...
    DEBUG("R_mm=%2.2f a_rad=%2.2f alpha_rad=%2.2f beta_rad=%2.2f t=%2.2f",
          R_mm, a_rad, alpha_rad, beta_rad, t);

    /* process the angular acceleration */
    Aa_rd_s2 = Va_rd_s / t;
    Aa = acc_rd2imp(traj, Aa_rd_s2);
    DEBUG("Aa_rd_s2=%2.2f Aa=%2.2f", Aa_rd_s2, Aa);

    /* exit if the robot cannot physically do it */
    if (Aa > Amax) {
        DEBUG("greater than max acceleration");
        return -1;
    }
    ...
    DEBUG("relative xm,ym = (%2.2f, %2.2f)",
          xm, ym);
    ...
    DEBUG("translate line1 by %2.2f,%2.2f", v.x, v.y);
    ...
    DEBUG("translate line2 by %2.2f,%2.2f", v.x, v.y);
    ...
    if (intersect_line(&line1_int, &line2_int, &center) != 1) {
        DEBUG("cannot find circle center");
        return -1;
    }
    DEBUG("center=(%2.2f,%2.2f)", center.x, center.y);
    ...
    DEBUG("absolute M = (%2.2f, %2.2f)", M.x, M.y);

    /* project M on line 1 */
    proj_pt_line(&M, &line1, &proj);
    DEBUG("proj M = (%2.2f, %2.2f)", proj.x, proj.y);

    /* process remaining distance before start turning */
    remain_d_mm = d_inter_mm - (pt_norm(&proj, &intersect) + xm);
    DEBUG("remain_d=%2.2f", remain_d_mm);
    if (remain_d_mm < 0) {
        DEBUG("too late, cannot turn");
        return -1;
    }
...
static void start_clitoid(struct trajectory* traj)
{
    ...
    DEBUG("%s() Va=%2.2f Aa=%2.2f",
          __FUNCTION__, Va, Aa);
    delete_event(traj);
    d = fabs(R_mm * a_rad);
    d *= 3.; /* margin to avoid deceleration */
    __trajectory_goto_d_a_rel(traj, d, a_rad,
                              RUNNING_AD, UPDATE_A | UPDATE_D);
