...
\section{Design of the EKF filter}

\begin{enumerate}
    \item What goes in the state ?
    \item What is a control input?
...
The measurement equation is the distance to a beacon at a known location $\mathbf{b}$.

\begin{equation}
    h(\mathbf{x}) = \sqrt{\left(\mathbf{x}_x - \mathbf{b}_x\right)^2 + \left(\mathbf{x}_y - \mathbf{b}_y\right)^2}
\end{equation}
